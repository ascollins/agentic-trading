...
class PortfolioAllocator:
    ...
    def _estimate_price(target: TargetPosition) -> float:
        """Get price estimate for notional calculation.

        Uses the ``price_estimate`` field populated by
        :class:`PortfolioManager` from the best signal's
        ``risk_constraints["price"]`` or ``features_used["close"]``.
        Falls back to ``1.0`` for legacy targets that predate this field.
        """
        if hasattr(target, "price_estimate") and target.price_estimate > 0:
            return target.price_estimate
        return 1.0
